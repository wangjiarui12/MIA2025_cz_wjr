...
class PromptTrainDataset(Dataset):
    def __init__(self, args):
        ...
        self.rs_ids = []
        self.hazy_ids = []
        self.lq_dir = '/home/user7/workspace1/Ultrasound-Enhancement/train_datasets/all_low_quality'
        self.hq_dir = '/home/user7/workspace1/Ultrasound-Enhancement/train_datasets/all_high_quality'
    ...
    def _init_clean_ids(self):
        ref_file = '/home/user7/workspace1/PromptIR/file_paths.txt'
        with open(ref_file, 'r') as f:
            temp_ids = [line.strip() for line in f if line.strip()]

        clean_ids = [
            path for path in temp_ids
        ]
    ...
    def _init_enhance_ids(self):
        # for enhance, list all files in HQ directory
        names = os.listdir(self.hq_dir)
        self.enhance_ids = [{'clean_id': os.path.join(self.hq_dir, n), 'de_type': self.de_dict['enhance']} for n in names]
    ...
    def __getitem__(self, idx):
        ...
        if de_id == 6:
            # Use provided LQ and HQ
            lq_path = os.path.join(self.lq_dir, os.path.basename(clean_path))
            lq_img = np.array(Image.open(lq_path).convert('RGB'))
            hq_img = np.array(Image.open(clean_path).convert('RGB'))
            # joint crop & augment
            img1, img2 = self._crop_patch(crop_img(lq_img, base=16), crop_img(hq_img, base=16))
            degrad_patch, clean_patch = random_augmentation(img1, img2)
            # print(degrad_patch)
            # print(clean_patch)
...
class PairedImageDataset(Dataset):
    def __init__(self, lq_dir, hq_dir, patch_size=None):
        super().__init__()
        self.lq_dir = lq_dir
        self.hq_dir = hq_dir
        self.patch_size = patch_size

        self.image_names = sorted(os.listdir(lq_dir))  # 假设两个文件夹下文件名一一对应
    ...
    def __getitem__(self, idx):
        ...
        hq_path = os.path.join(self.hq_dir, name)
        ...
        if self.crop_transform:
            ...
            lq_img = self.crop_transform(lq_img)
            torch.manual_seed(seed)
            hq_img = self.crop_transform(hq_img)

        lq_tensor = self.to_tensor(lq_img)
        hq_tensor = self.to_tensor(hq_img)

        clean_name = name.split('.')[0]
        de_id = 0  # 固定值，表示自带LQ，不是模拟降质

        return [clean_name, de_id], lq_tensor, hq_tensor
